...
export default async function SegmentoPage({ params }: Props) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-foreground">
      ...
      {/* Hero Section */}
      <section className="relative overflow-hidden lg:h-[90vh] pt-32 pb-20 md:pt-40 md:pb-28 bg-white flex flex-col items-center justify-center border-b border-secondary-100">
        <div className="mx-auto w-full max-w-7xl px-4 md:px-6 lg:px-8 relative z-10 flex flex-col items-center text-center">
          {/* Badge */}
          <div className="inline-flex items-center rounded-full border border-primary-500 bg-primary-50 px-4 py-1.5 text-xs text-primary-600 mb-6 font-display font-medium text-pretty">
            JobbLive para {data.name}
          </div>

          {/* Headline */}
          <h1 className="hero-animate-fade-up text-3xl md:text-5xl lg:text-6xl delay-1 max-w-4xl text-foreground text-pretty tracking-tightest leading-tightest mb-6">
            {data.heroTitle}
          </h1>

          {/* Subtítulo */}
          <p className="text-body-lg text-foreground/70 max-w-2xl text-pretty mb-10 leading-relaxed">
            {data.heroSubtitle}
          </p>

          {/* Botões de Ação */}
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
            <Link href="/teste-gratis" className="btn-primary btn-lg w-full sm:w-auto px-8 font-normal text-center">
              Começar teste de 7 dias
            </Link>
            <Link href="/agendar-demo" className="btn-secondary btn-lg w-full sm:w-auto px-8 font-normal text-center bg-white hover:bg-secondary-50">
        ...
        <div className="mx-auto w-full max-w-7xl px-4 md:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {data.painPoints.map((point, idx) => (
              <div key={idx} className="flex flex-col bg-white p-8 rounded-none border border-secondary-100">
