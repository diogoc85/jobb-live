...
export default function Hero() {
  return (
    <section id="hero" className="full relative overflow-hidden pt-32 pb-16 md:pt-44 md:pb-24 lg:pt-52 lg:pb-16 bg-background flex flex-col items-center justify-center">
      {/* Imagem de Fundo */}
      <Image
      ...
      <div className="mx-auto w-full max-w-7xl px-4 md:px-6 lg:px-8 relative z-10 flex flex-col items-center text-center">
        ...
        {/* Badge / Tag Superior */}
        <div className="hero-animate-fade-up inline-flex items-center rounded-full border border-white px-4 py-1.5 text-xs text-white mb-6">
          Agências <span className="mx-2 opacity-50">|</span> Marketing <span className="mx-2 opacity-50">|</span> Eventos
        </div>

        {/* Título Principal */}
        <h1 className="hero-animate-fade-up delay-1 max-w-4xl text-foreground text-pretty tracking-tightest leading-tightest mb-6">
          Software de gestão para <br className="hidden md:block" />
          <span className="text-primary-500">agência de marketing.</span>
        </h1>

        {/* Subtítulo / Descrição */}
        <p className="hero-animate-fade-up delay-2 text-heading-5 text-foreground/70 max-w-2xl text-pretty mb-10">
          Gerencie projetos, financeiro e operação em um único sistema,
          sem planilhas, sem retrabalho.
        </p>

        {/* Botões de Ação */}
        <div className="hero-animate-fade-up delay-3 flex flex-col sm:flex-row items-center gap-4 mb-5 w-full sm:w-auto">
          <Link href="/teste-gratis" className="btn-primary btn-lg w-full sm:w-auto px-8">
            Teste e conheça o JobbLive
          </Link>
          <Link href="/agendar-demo" className="btn btn-lg w-full sm:w-auto px-8 bg-white text-foreground hover:bg-secondary-50">
            Agendar demo
          </Link>
        </div>

        {/* Tags de Segurança / Info */}
        <p className="hero-animate-fade-up delay-4 text-body-sm text-foreground/50 mb-16 md:mb-20">
        ...
        <div className="hero-animate-fade-up delay-5 w-full max-w-5xl flex flex-col items-center pt-8 border-t border-foreground/10 select-none">
